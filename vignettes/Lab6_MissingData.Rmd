...
knitr::include_graphics("https://en.wikipedia.org/wiki/European_hare#/media/File:Lepus_europaeus_(Causse_M%C3%A9jean,_Loz%C3%A8re)-cropped.jpg")
```
...
2. When this data was formally analyzed, the authors used a random effect of year in the process model and a random effect of site in the detection process. Run the model with both of these random effects. Use ggplot to make a plot of the beta1 values (the effect of landuse type on the expected abundance) produced by both models. 

3. Since this is an abundance model, it makes sense to make a plot of abundance over time. Inside the model that you used for Question 2, create a *derived parameter* that represents the total abundance of all sites of each landtype in each year. Use ggplot to graph the estimated total abundance by landtype over time, including the CIs.  
