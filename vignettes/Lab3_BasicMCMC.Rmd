...
We also need a model for $\sigma$. A lot of MCMC codes (such as the BUGS language) model a normal distribution as $Normal(mean, precision)$ where precision = ($1/\sigma^2$). To get use to this, we will also model the precision ($\tau$) noting that:
